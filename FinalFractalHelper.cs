...
namespace Zenith
{
	...
	public struct FinalFractalHelper
	{
		...
		public struct FinalFractalProfile
		{
			...
			private void StripDust(Vector2 centerPosition, float rotation, Vector2 velocity)
			{
				if (Main.rand.Next(9) == 0)
				{
					...
					for (int i = 0; i < num; i++)
					{
						Dust dust = Dust.NewDustPerfect(centerPosition, ModContent.DustType<ZenithDust>(), null, 100, Color.Lerp(trailColor, Color.White, Main.rand.NextFloat() * 0.3f));
						dust.scale = 0.4f;
		...
		public void Draw(Projectile proj)
		{
			...
			MiscShaderData14 miscShaderData = ModContainer.Instance.finalFractal;
			int num = 4;
			int num2 = 0;
			int num3 = 0;
			int num4 = 4;
			miscShaderData.UseShaderSpecificData(new Vector4(num, num2, num3, num4));
			miscShaderData.UseImage0(ModContainer.Instance.GetTexture("Extra_201"));
			miscShaderData.UseImage1(ModContainer.Instance.GetTexture("Extra_193"));
